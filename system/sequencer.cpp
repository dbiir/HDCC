...
void Sequencer::process_ack(Message * msg, uint64_t thd_id) {
#if CC_ALG == MIXED_LOCK
	if (msg->get_batch_id() == simulation->get_seq_epoch()+1) {
	...
	AckMessage* ack_msg = (AckMessage*)msg;
#else
...
#if CC_ALG == MIXED_LOCK
			if ((msg->algo == SILO && ack_msg->isCommit) || msg->algo == CALVIN) {
#endif
...
void Sequencer::process_txn(Message *msg, uint64_t thd_id, uint64_t early_start,
		...
		if (id >= en->max_size) {
			en->max_size *= 2;
			en->list = (qlite *) mem_allocator.realloc(en->list,sizeof(qlite) * en->max_size);
			INC_STATS(thd_id, seq_realloc_cnt, 1);
		}
#else
		next_txn_id++;
		...
		if (cc_selector.get_best_cc(_wl, msg) == SILO) {
			...
			// We only let the node that initiates 2pc to send the ack to let sequencer know if a txn is commited or aborted.
			en->list[id].server_ack_cnt = 1;
			en->list[id].msg = msg;
			en->size++;
			en->txns_left++;
			en->list[id].msg = msg;
			// Note: Modifying msg!
			msg->original_return_node_id = msg->return_node_id;
